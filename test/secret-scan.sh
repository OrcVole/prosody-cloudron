#!/bin/bash
# Anonymity gate (prosody: highest-stakes of the fleet — an XMPP config names hosts by nature) — no box hostname, private-mirror URL, internal stack URL, token, or personal
...
patterns=(
  ...
  # prosody-specific: an XMPP package's config, MUC aliases and TURN settings all name hosts,
  # so a leak here is likelier than in any other package on the estate.
  'conference\.haggis'
  'xmpp\.haggis'
  'turn(server)?\.[a-z0-9-]*\.(top|win|dev)'
  '[a-z0-9._%+-]+@haggis\.top'
)
